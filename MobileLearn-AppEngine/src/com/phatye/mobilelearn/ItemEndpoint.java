...
import com.phatye.mobilelearn.PMF;
...
@Api(name = "itemendpoint", namespace = @ApiNamespace(ownerDomain = "phatye.com", ownerName = "phatye.com", packagePath = "mobilelearn"))
public class ItemEndpoint {
	...
	public CollectionResponse<Item> getItems(@Named("category") String category, @Named("type") String type) {
		PersistenceManager mgr = getPersistenceManager();
		//Item[] items = null;
		try {
			/*Query q = mgr.newQuery("Select t from Item t where t.category = :cat " +
					"and t.type = :typ");
			q.declareParameters(category);
			q.declareParameters(type);*/
			
			Query query = mgr.newQuery(Item.class);
			query.setFilter("category == categoryParam && type == typeParam");
			query.setOrdering("title desc");
			query.declareParameters("String categoryParam, String typeParam");
			
			List<Item> itemList = (List<Item>)query.execute(category, type);
			return CollectionResponse.<Item> builder().setItems(itemList).build();
		} finally {
			mgr.close();
		}
		
		//return new CollectionResponse<Item>();
	}
